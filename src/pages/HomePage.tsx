...
export function HomePage() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="relative h-[calc(100vh-136px)] md:h-[calc(100vh-72px)] min-h-[500px] overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center max-w-3xl mx-auto px-4">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }} className="hidden md:block text-[#E8552A] uppercase tracking-[0.25em] text-sm font-semibold mb-4">{settings.heroTagline}</motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.7 }} className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mt-6 sm:mt-8">
            <Link to="/products" className="bg-white text-[#1A5A6B] px-6 py-3 sm:px-8 sm:py-4 rounded-full font-semibold text-sm sm:text-base hover:bg-gray-100 transition-all hover:scale-[1.02] shadow-lg inline-block">{settings.heroCtaPrimary}</Link>
            <Link to="/products" className="border-2 border-white text-white px-6 py-3 sm:px-8 sm:py-4 rounded-full font-semibold text-sm sm:text-base hover:bg-white/15 transition-all inline-block">{settings.heroCtaSecondary}</Link>
          </motion.div>
        </div>
        <motion.button
          initial={{ opacity: 0 }}
          ...
          className="absolute bottom-6 sm:bottom-8 left-1/2 -translate-x-1/2 text-white/60 flex flex-row items-center gap-2 cursor-pointer hover:text-white transition-colors bg-transparent border-none"
        >
      ...
      {/* Best Sellers */}
      <section id="best-sellers" className="py-6 md:py-20 bg-white scroll-mt-[72px]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-start md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-10">
            <div className="text-left">
              <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.bestSellers.title}</h2>
              <p className="text-[#6B7280] mt-2 text-sm">{settings.sectionTitles.bestSellers.subtitle}</p>
            </div>
            <Link to="/products" className="text-[#1A5A6B] font-medium text-sm hover:underline flex items-center gap-1 self-start md:self-auto">View All <ChevronDown className="w-4 h-4 rotate-[-90deg]" /></Link>
          </div>
          <SectionCarousel itemsPerView={{ mobile: 1, tablet: 2, desktop: 4 }}>
      ...
      {/* Categories CTA */}
      <section className="py-6 md:py-16 bg-[#F5F0E8]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.categories.title}</h2>
      ...
      <section className="py-6 md:py-16 bg-[#F5F0E8]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-left mb-6 md:mb-10">
            <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.promotions.title}</h2>
            <p className="text-[#6B7280] mt-2">{settings.sectionTitles.promotions.subtitle}</p>
          </div>
          <SectionCarousel itemsPerView={{ mobile: 1.15, tablet: 2, desktop: 3 }}>
            {visiblePromotions.map((promo) => (
              <Link key={promo.id} to={promo.link} className={`group block bg-gradient-to-br ${promo.gradient} rounded-2xl p-6 text-white h-full hover:shadow-xl transition-all hover:-translate-y-1`}>
      ...
      {settings.productBundles.filter(b => b.active).length > 0 && (
        <section className="py-6 md:py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-left mb-6 md:mb-10">
              <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">Bundle Deals</h2>
              <p className="text-[#6B7280] mt-2">Buy more, save more — exclusive bundle pricing</p>
            </div>
            <SectionCarousel itemsPerView={{ mobile: 1.15, tablet: 2, desktop: 3 }}>
              {settings.productBundles.filter(b => b.active).map((bundle) => {
                const bundleProducts = activeProducts.filter(p => bundle.productIds.includes(p.id)).slice(0, 4);
                return (
                  <Link key={bundle.id} to="/products" className="group block bg-gradient-to-br from-[#1A5A6B] to-[#1A8DA3] rounded-2xl p-6 text-white h-full hover:shadow-xl transition-all hover:-translate-y-1">
                    <div className="flex items-center justify-between mb-4">
      ...
      <section className="py-6 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-start md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-10">
            <div className="text-left">
              <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.newArrivals.title}</h2>
              <p className="text-[#6B7280] mt-2 text-sm">{settings.sectionTitles.newArrivals.subtitle}</p>
            </div>
            <Link to="/products" className="text-[#1A5A6B] font-medium text-sm hover:underline flex items-center gap-1 self-start md:self-auto">View All <ChevronDown className="w-4 h-4 rotate-[-90deg]" /></Link>
          </div>
          <SectionCarousel itemsPerView={{ mobile: 1.15, tablet: 2, desktop: 4 }}>
      ...
      <section className="py-6 md:py-20 bg-[#F0F7F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-left mb-6 md:mb-12">
            <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.reviews.title}</h2>
            <p className="text-[#6B7280] mt-3">{settings.sectionTitles.reviews.subtitle}</p>
          </div>
          <SectionCarousel itemsPerView={{ mobile: 1.15, tablet: 2, desktop: 2 }}>
            {(featuredReviews.length > 0 ? featuredReviews : settings.reviewHighlights).map((review) => (
              <Link key={review.id} to={`/product/${review.productId}`} className="flex flex-col bg-white rounded-2xl p-6 hover:shadow-lg transition-all border border-gray-100 h-full">
                ...
                <div className="flex items-start gap-3 mb-3 flex-1">
                  ...
                  <p className="text-[#1A1A1A] text-sm leading-relaxed line-clamp-3 h-[4.5rem]">{review.text}</p>
                </div>
                <div className="flex flex-col gap-2 pt-4 border-t border-gray-50 mt-auto">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-[#1A5A6B] text-white flex items-center justify-center text-xs font-bold">
                      {(review as any).userName ? (review as any).userName.charAt(0) : (review as any).name.charAt(0)}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-[#1A1A1A]">{(review as any).userName || (review as any).name}</p>
                      <p className="text-xs text-gray-400">{review.date}</p>
                    </div>
                  </div>
                  <span className="self-start text-xs text-[#1A5A6B] bg-[#1A5A6B]/10 px-3 py-1 rounded-full font-medium truncate max-w-full">
      ...
      {/* Bundle Deal Banner */}
      <section className="py-6 md:py-16 bg-gradient-to-r from-[#E8552A] to-[#C4451D]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            ...
            <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-white">{settings.sectionTitles.bundleBanner.title}</h2>
      ...
      {/* Brand Promise */}
      <section className="py-6 md:py-20 bg-[#F5F0E8]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-left mb-6 md:mb-12">
            <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.brandPromise.title}</h2>
            <p className="text-[#6B7280] mt-3">{settings.sectionTitles.brandPromise.subtitle}</p>
          </div>
          <SectionCarousel itemsPerView={{ mobile: 1, tablet: 2, desktop: 4 }}>
            {settings.brandPromises.map((f) => (
      ...
      {/* Join the Wild Club */}
      <section className="py-6 md:py-16 bg-[#1A4A52]">
  ...
  return (
    <section className="py-6 md:py-20 bg-[#F0F7F5]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-start md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-10">
          <div className="text-left">
            <h2 className="font-heading text-lg md:text-2xl lg:text-3xl font-bold text-[#1A1A1A]">{settings.sectionTitles.recommended.title}</h2>
